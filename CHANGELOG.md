...
* Added `InsertStatement::execute_returning_id` for MySQL, returning the `AUTO_INCREMENT` value an insert set (`None` if it set none) without the extra `SELECT LAST_INSERT_ID()` round trip
