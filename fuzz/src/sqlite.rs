...
/// compared: `json()` prints a double to fifteen significant digits, which does
/// not round trip, so a comparison against it reports sqlite's loss as a defect.
...
/// `json_valid(?, 8)`: conformance to sqlite's own jsonb format. A query error
/// is no verdict on the blob, so it stays an error rather than becoming one.
...
// one macro call, so the case list and the dispatch cannot drift apart
macro_rules! cases {
    ( $( ($name:literal, $ST:ty, $T:ty) ),* $(,)? ) => {
        /// The case names, indexed by selector.
        pub const CASES: &[&str] = &[ $( $name ),* ];

        /// Decodes a bound value with the selected impl.
