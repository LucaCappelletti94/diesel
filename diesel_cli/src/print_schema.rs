...
impl Display for CustomTypesForTablesForDisplay<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self.custom_types.backend {
            ...
            #[cfg(feature = "mysql")]
            Backend::Mysql => self.mysql_like_display(f),
            #[cfg(feature = "mariadb")]
            Backend::Mariadb => self.mysql_like_display(f),
        }
    }
}

impl<'a> CustomTypesForTablesForDisplay<'a> {
    #[cfg(any(feature = "mysql", feature = "mariadb"))]
    fn mysql_like_display(&self, f: &mut Formatter<'_>) -> Result<(), fmt::Error> {
        ...
        let mut types_to_generate: Vec<(&ColumnType, &TableName, &ColumnDefinition)> =
            types_overrides_sorted
                ...
                .flat_map(|(ct, t)| {
                    ct.iter()
                        .zip(t.columns())
                        .filter_map(move |(ct, c)| ct.as_ref().map(|ct| (ct, t.table_name(), c)))
                })
                .collect();
        if types_to_generate.is_empty() {
            return Ok(());
        ...
        for (idx, &(custom_type, table, column)) in types_to_generate.iter().enumerate() {
            let enum_type = if let Some(variants) =
                crate::infer_schema_internals::mysql_like::get_enum_variants(&column.ty)
            {
                rust_types.push(RustEnum {
                    tpe: custom_type,
                    variants,
    ...
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        {
            ...
            writeln!(
                out,
                "pub mod {} {{",
                crate::infer_schema_internals::rust_name_for_sql_name(self.0, None)
            )?;
